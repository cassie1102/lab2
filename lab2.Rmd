...
```{r}
str(senators$object[[1]])
senators$objects[[1]]$person$birthday
```
...
```{r}
birth.state <- data.frame(rep(0, length(senators$objects)), rep(0, length(senators$objects), stringsAsFactors = FALSE))
colnames(birth.state) <- c("state", "birthday")
for (i in 1:length(senators$objects)){
  birth.state[i, 1] = senators$objects[[i]]$state
  birth.state[i, 2] = senators$objects[[i]]$person$birthday
}
birth.state$birthday[birth.state$state == "NC"]
```
...
```{r}
leader.twitter <- data.frame(matrix(nrow = 1, ncol = 2))
colnames(leader.twitter) <- c("leadership", "twitter ID")
for(i in 1:length(senators$objects)){
  if(is.null(senators$objects[[i]]) == F){
    leader.senator <- c(senators$objects[[i]]$leadership_title, senators$objects[[i]]$person$twitterid)
    leader.twitter <- rbind(leader.twitter, leader.senator)
  }
}
```
